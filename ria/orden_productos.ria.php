...
if (!strlen($id_user)) {
	...
	$aColumns = array('t1.IdCotizacionDt', 't3.Categoria', 't2.Descripcion', 't1.Cantidad', 't1.Costo', 't1.IdCotizacionDt AS icons');
	$aColumnsClean = array( 'IdCotizacionDt', 'Categoria', 'Descripcion', 'Cantidad', 'Costo', 'icons');

	// especifico
	// ..
	
	/* Indexed column (used for fast and accurate table cardinality) */
	$sIndexColumn = "IdCotizacionDt";
	
	/* DB table to use */
	$sTable = "invcotizacionDt";
	
	// custom from
	$custom_from = "FROM invcotizacionesDt t1
    LEFT JOIN productos t2 ON t1.IdArticulo_fk = t2.IdArticulo
	...
	while ( $aRow = mysqli_fetch_array( $rResult ) ) {
		...
		for ( $i=0 ; $i<count($aColumns) ; $i++ ) {
			if ( $aColumns[$i] == "icons" ) {
                ...
                $icons = '<button type="button" class="btn btn-danger btn-delete btn-sm"><i class="fas fa-trash-alt"></i></button>';
				$row[] = $icons;
            ...
            //     }
            //     $row[] = $html;
            } else if ($aColumns[$i] == "Descripcion") {
                $descripcion = $aRow[ $aColumns[$i] ];
                $row[] = utf8_encode($descripcion);
			} else if ( $aColumns[$i] != ' ' ) {
				/* General output */
