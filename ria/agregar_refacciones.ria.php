...
if (!strlen($id_user)) {
	...
	$aColumns = array('t1.IdProducto', 't1.Referencia', 't1.NombreRefaccion', 't2.Categoria', 't1.IdProducto AS Cantidad', 't1.IdProducto AS icons');
	$aColumnsClean = array('IdProducto','Referencia', 'NombreRefaccion', 'Categoria', 'Cantidad', 'icons');
	...
	while ( $aRow = mysqli_fetch_array( $rResult ) ) {
		...
		for ( $i=0 ; $i<count($aColumns) ; $i++ ) {
				...
				$id_producto = $aRow[ 'IdProducto' ];
			
			if ( $aColumns[$i] == "Cantidad" ) {
				$icons = "<input type='text' class='form-control form-control-sm valores' name='holis+$id_prodcuto' data-id='$id_prodcuto'>";
				$row[] = $icons;
